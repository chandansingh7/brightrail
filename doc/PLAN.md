...
**Fun Angular Component Library** (abbreviated **FACL** in tooling where short names help) is an Angular workspace with a **publishable UI library** and a **funfair** app: a lightweight, static showcase where you try components in the browser (not named “demo” on purpose—it’s the fun fairgrounds for FACL).
...
The **installable artifact** is not the `projects/facl` source folder; it is whatever **`ng build facl`** writes under **`dist/facl/`** (Angular Package Format via **ng-packagr** and your library’s `ng-package.json`).

| Question | Answer |
|----------|--------|
| **When is it created?** | Whenever you run **`ng build facl`** — locally before publish, as the **first step** of **`npm run verify:package`**, and in **CI** on every change you care about. |
| **How is it created?** | The CLI compiles **`projects/facl`** (from `public-api.ts` / `src/lib/`) into **`dist/facl/`**: a folder with **`package.json`**, compiled JS bundles (e.g. FESM), **`.d.ts`**, and Angular metadata — the **same layout** that ends up on the registry after publish (exact file list follows your Angular/ng-packagr version). |
| **When do people `npm install` it?** | **Externally:** after you **`npm publish`** (from `dist/facl` or your publish step). **In this repo:** **`Midway`** does not pull from the registry; root **`package.json`** uses **`"@your-scope/facl": "file:dist/facl"`**, then **`npm install`** links that folder into **`node_modules`** so **`Midway`** behaves like a real consumer. |

So: **build = package appears in `dist/facl`**; **`verify:package`** builds, reinstalls, then **builds `Midway`** to prove that artifact works.
...
3. **Midway** — A **minimal Angular app** (`projects/midway`) in the same repo that depends on the **built** library under `dist/facl` via **`file:`** (or `npm pack` + install). It must **not** use TypeScript `paths` to `projects/facl/src`, so resolution matches a real **`npm install`**.
...
Design **facl** so people are not blocked by relying on a mouse or precise pointing alone:

| Audience | What facl must provide |
|----------|-------------------------|
...
The clone folder on disk can be anything (for example `facl-ui`); the **product name** is Fun Angular Component Library.
...
├── projects/
│   ├── facl/                   # Angular library project (Fun Angular Component Library)
│   │   ├── src/
│   │   │   ├── lib/
│   │   │   └── public-api.ts # Public exports only
│   │   └── ng-package.json
│   ├── funfair/              # Showcase — path-mapped to facl source for fast dev
│   │   └── src/
│   │       ├── app/
│   │       └── index.html
...
│           ├── app/
│           └── index.html
├── angular.json              # Build targets: facl + funfair + midway
...
**Path mappings (important):** After scaffolding, configure TypeScript **`paths`** for `@your-scope/facl` **only** under **`funfair`** (e.g. `projects/funfair/tsconfig.*`), **not** in the workspace root. **Midway** should have **no** path override so imports resolve through **`node_modules`** → **`file:dist/facl`**, matching what publishers get after `npm install`.

**Naming:** The in-repo library project is **`facl`** (`ng build facl`). The showcase is **`funfair`**. The consumer-smoke app is **`midway`** (project folder `projects/midway`, display name **Midway**). When you publish the library, pick an npm name you prefer, for example `@your-scope/facl` or `@your-scope/fun-angular-component-library`, and align `package.json` / `public-api.ts` imports with that choice.

### How funfair wires to `facl` (same repo — not `npm install` of your own package)

In a single Angular **workspace**, **funfair** does **not** need to run `npm install @your-scope/facl` from the registry to use the library you are building next door. The CLI sets up **TypeScript path mappings** (and workspace dependencies, depending on version) so funfair imports the library **by its package name** (e.g. `@your-scope/facl`) and the tooling resolves that to **`projects/facl`** (or the built output under `dist/`) during **`ng serve funfair`** and local builds.

- **Inside this repo:** one root `npm install` for third-party deps; develop `facl` and consume it from funfair **without** publishing first.
- **Outside this repo:** other applications add your library with **`npm install @your-scope/facl`** (or yarn/pnpm) **after** you publish to a registry (or a private feed).

So: **`projects/facl`** is what you **build and publish**; **`projects/funfair`** is the app that **imports** it like a consumer — locally via the workspace link, and for the rest of the world via **npm** after release.

### Consumer verification in one command (Midway + `verify:package`)

**Midway** is a minimal Angular application whose only job is to **import `@your-scope/facl` from the built output** (same as after `npm install`).

**Root `package.json` (pattern):**

- Add a dependency: `"@your-scope/facl": "file:dist/facl"` (use your real scope/name).
- Add a script, e.g.  
  `"verify:package": "ng build facl && npm install && ng build midway"`  

**What one run does:**

1. **`ng build facl`** — Produces / refreshes `dist/facl` with the package shape you publish.
2. **`npm install`** — Links `node_modules/@your-scope/facl` to that folder (consumer layout).
3. **`ng build midway`** — Compiles the tiny app using **only** the packaged library (no `projects/facl/src` path hack, if you scoped `paths` to funfair only).
...
Funfair importing `facl` via **path-mapped** dev is fast, but it does not prove the **tarball / `dist` layout** by itself. The **`verify:package`** flow above is the **primary** in-repo guarantee; the list below is for extra confidence or CI variants.

1. **Always ship from a clean library build** — `ng build facl`, publish what ng-packagr outputs (`dist/facl` or your configured path). CI should run **`verify:package`** (or at least `ng build facl`) on every main-branch change.
2. **Optional:** Build funfair with overrides so it resolves **only** `dist/facl` (redundant if **Midway** exists, but still valid).
3. **Optional:** `npm pack` on `dist/facl`, install the `.tgz` in a **throwaway** app outside the repo for a fully external smoke test.
...
| **Midway** | Minimal app: depends on **`file:dist/facl`**, no path mapping to source; ride-tests **`verify:package`**. |
...
2. **Develop the library** under `projects/facl/src/lib/`.
3. **Run the showcase** with `ng serve funfair` (or `npm run serve:funfair`).
4. **Prove the package in one go** with `npm run verify:package` (builds `facl`, runs `npm install`, builds **Midway**) — use before publish and in CI.
5. **Build the library** alone with `ng build facl` when you need `dist/` for publishing.
...
- **`funfair` is not part of that install.** `npm install @your-scope/facl` delivers only the **library** package. The showcase app stays in **your Git repository** and/or a **separately deployed** static site—not inside node_modules for consumers.
...
- **Secondary entry points** (e.g. `@your-scope/facl/button`) if the package grows and you want tree-shakable subpaths.
...
- [ ] Create Angular workspace with **`facl`**, **`funfair`**, and **`midway`** (minimal app).
...
- [ ] Scope **`paths`** for `@your-scope/facl` to **funfair only**; keep **Midway** on **`file:dist/facl`** + `node_modules` resolution.
...
**Fun Angular Component Library** is maintained as **one codebase** with **three coordinated targets**: the **library** (`facl`), the **funfair** showcase (path-mapped dev), and **Midway** (`projects/midway` — one-command **npm-shaped** proof via **`verify:package`**). You ship `facl` to npm; funfair and Midway stay in-repo for validation and docs.
